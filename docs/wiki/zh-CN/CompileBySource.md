...
```shell
git clone git@github.com:DwArFeng/scg.git
```

对于中国用户，可以使用 gitee 进行下载。

```shell
git clone git@gitee.com:dwarfeng/scg.git
```
...
`scg-distribute` 模块会将各节点产物汇总到统一的输出目录（见下一节）。

如果上述命令执行失败，请仔细阅读报错内容，绝大部分情况下是因为上述依赖缺失。请您重复上述步骤，直到编译成功。

## 寻找打包后的目标文件

运行节点按功能与构型划分；各节点在 `scg-node` 下以子模块形式维护。

以 `scg-node-all-he` 为例：

- `all` 表示功能维度（与构建中的 `node.function` 一致），
- `he` 表示构型维度（与 `node.configuration` 一致）。

若日后增加其它节点，`scg-distribute` 的装配可能增加对应的汇总子目录，请以该模块的装配描述为准。

推荐用于确认打包成功的主路径（各节点发布物的汇总目录）：

```
scg-distribute/target/distribute
```
...
- `scg-all-he/`
   - 压缩包形态文件 `scg-all-he-${version}-release.tar.gz`
   - 解压形态目录 `scg-all-he-${version}-release/`
...
- `scg-node-all-he/`
   - 压缩包形态文件 `target/scg-all-he-${version}-release.tar.gz`
   - 解压形态目录 `target/scg-all-he-${version}-release/`

| 模块名称              | 制品路径                                    |
|:------------------|:----------------------------------------|
| `scg-node-all-he` | `target/scg-all-he-${version}-release/` |

对交付与发布而言，以 `scg-distribute/target/distribute` 下的发布包与目录为准。
