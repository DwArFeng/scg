...
logic-engine-start.sh 是本项目的启动脚本，用于启动本项目。 您可以调整脚本中的参数，以适应您的需求。
...
脚本执行后，将会以 nohup 方式启动本项目，并将 PID 写入 logic-engine.pid 文件中。
...
## logic-engine-stop.sh

### 简介

logic-engine-stop.sh 是本项目的停止脚本，用于停止本项目。

脚本执行后，将会读取 logic-engine.pid 文件中的 PID，并使用 kill 命令停止本项目。
